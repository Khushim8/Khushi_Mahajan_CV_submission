...
Image classifier for industrial metal surface defects (crack / hole / rust / scratch / normal). Built for the Rigetti internship assessment.
...
dataset should live at `dataset/` relative to the project root:

```
dataset/
  train/  crack/  hole/  normal/  rust/  scratch/
  test/   crack/  hole/  normal/  rust/  scratch/
```

## running

```bash
# full pipeline: train -> eval -> predictions.csv
python main.py

# skip training and just run eval + inference (needs a saved checkpoint)
python main.py --skip-train

# retrain from a saved checkpoint on new labeled images
python main.py --retrain path/to/new_labeled_data --skip-train
```

outputs:
- `predictions.csv` in the repo root
- `outputs/best_model.pth` checkpoint
